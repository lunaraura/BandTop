...
class Camera {
    ...
    projectTo3D(worldVertices) {
        ...
        for (const id in worldVertices) {
            ...
            if (Math.abs(denom) < 1e-6) continue; // avoid divide-by-zero; skip vertices too close to camera in w
            const wScale = this.wDistance / denom;
            out[id] = {
                pos: v(p.x * wScale, p.y * wScale, p.z * wScale, 0),
                w: p.w
            };
        }
        return out;
    ...
    raycastOverlay(object, resX, resY = 1) {
        ...
        const layers = [];
        const steps = 9;
        const maxOffset = this.wTolerance;
        for (let i = 0; i < steps; i++) {
            const u = i / (steps - 1);
            const offset = (u * 2 - 1) * maxOffset;
            this.pos.w = oldW + offset;
            const hits = this.raycastAtW(object, resX, resY, this.pos.w);
            layers.push(hits);
        }
        this.pos.w = oldW;
        return layers;
    }
    raycastAtW(object, resX, resY = 1, cameraW) {
        const oldW = this.pos.w;
        this.pos.w = cameraW;
    ...
    castAgainstProjected(object, rays, camVerts) {
        ...
        if (this.configuration === "3D") {
            ...
            if (hitTri) {
                hits.push({ tri: hitTri, t: closestT, w: closestW });
            }
        }
        return hits;
    } else {
        ...
        for (const ray of rays) {
            ...
            if (hitEdge) {
                hits.push({ edge: hitEdge, t: closestT, w: closestW });
            }
        }
        return hits;
    }}
