...
const BlockPlacerTool = {
  ...
  drawOverlay(ctx, player){
    const placeType = PLACEABLE_TYPES[player.inventory.placeIndex];
    const mw = player.mouseWorld();
    const { tx, ty } = worldToTile(mw.wx, mw.wy);
...
class Player {
  ...
  handleClickOnce(scene){
    if (!this.mouse.clicked) return;
    if (scene.hitboxCheck) {
        const hit = scene.hitEntity(this.mouseWorld().wx, this.mouseWorld().wy);
        if (hit) {
            scene.onHitEntity(hit, this);
            return;
        }
    }
  }
...
class Toolbelt {
    ...
    prev(){ this.player.inventory.toolIndex = (this.player.inventory.toolIndex - 1 + this.tools.length) % this.tools.length; }
    update(dt){
        const t = this.active;
        if (!t) return;
    ...
    drawOverlay(ctx){
        const t = this.active;
        if (t?.drawOverlay) t.drawOverlay(ctx, this.player);
    }
}
...
class Drawer {
    constructor(cam, canvas, scene){
        this.camera = cam;
        this.canvas = canvas;
        this.scene = scene;
    }
    drawVisibleWorld(camera) {
...
class World {
  ...
  decorateChunk(chunk) {
    ...
    for (let ly = 0; ly < chunk.size; ly++) {
        ...
        this.ensureTree(tx, ty);
        this.ensureFlowerAt(tx, ty);
        }
    }
  }
  ...
  ensureTree(tx, ty) {
    if (!this.shouldHaveTree(tx, ty)) return;
...
function shouldAdvanceStory(player) {
    ...
    if (player.wasPressed(" ")) return true;
...
TOOLBELT.tools = toolFactories;

// Global inventory helpers
function invGet(type){ return player.inventory.blocks.get(type) ?? 0; }
function invAdd(type, n = 1){ player.inventory.blocks.set(type, invGet(type) + n); }
function invSpend(type, n = 1){
  const have = invGet(type);
  if (have < n) return false;
  player.inventory.blocks.set(type, have - n);
  return true;
}
...
function loop(now) {
  ...
  if (player.wasPressed("q")) TOOLBELT.prev();
  if (player.wasPressed("e")) TOOLBELT.next();
  if (player.wasPressed("r") && TOOLBELT.active === BlockPlacerTool) BlockPlacerTool.cycleNext();
  ...
  SM.drawer.drawHUD(player.inventory);
  drawExtra(now);

  SM.drawOverlay(ctx);

  player.beginFrame();
  player.mouse.clicked = false;
